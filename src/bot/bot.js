...
class Bot {
  constructor(difficulty, rank, debug) {
    this.rank = rank;
  ...
  createBot() {
    ...
    botData.name = botConff.names[
      Math.floor(Math.random() * botConff.names.length)
    ].toUpperCase();
    ...
    if (this.debug) console.log("Creating bot", botData);
  }
  ...
  mannager() {
    this.socket.on("start", () => {
      ...
      this.socket.on("gamewinner", (winner) => {
        if (this.debug) console.log("Game Ended: ", winner);
        this.clearTimeOuts();
        this.socket.disconnect();
      });

      this.socket.on("left", () => {
        if (this.debug) console.log("Game Ended: ", "other user left");
        this.clearTimeOuts();
        this.socket.disconnect();
      });
    });
  ...
  answer(timeout) {
    let delay =
      botConff.level[this.botLevel].timeOutsAfterPlaying[
        Math.floor(
          Math.random() *
            botConff.level[this.botLevel].timeOutsAfterPlaying.length
        )
      ];
    ...
    let delta = max - min;
    let answer =
      botConff.level[this.botLevel].rightAnswer[
        Math.floor(
          Math.random() * botConff.level[this.botLevel].rightAnswer.length
        )
      ];
    ...
    if (this.debug) console.log("Bot answered: ", answer);
  }
